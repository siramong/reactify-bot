...
class Logger {
  ...
  private teacherChannelId: string | undefined;
   ...
   */
  async getTeacherChannel(): Promise<TextChannel | null> {
    ...
    try {
      const channel = await this.client.channels.fetch(this.teacherChannelId);
        ...
        case 'ACTIVITY_CREATED':
          embed
            ...
            .addFields(
              ...
              { name: 'Nivel', value: data.nivel!, inline: true },
              { name: 'Recompensa', value: data.reward && data.reward > 0 ? `${data.reward} monedas` : 'Sin recompensa', inline: true },
              { name: 'Creado por', value: `<@${data.performedBy}>`, inline: true }
            );
          break;
